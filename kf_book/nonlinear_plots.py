...
def plot_bivariate_colormap(xs, ys):
    ...
    plt.gca().imshow(np.rot90(Z), cmap=plt.cm.Greys, extent=[xmin, xmax, ymin, ymax])
...
def plot_monte_carlo_mean(xs, ys, f, mean_fx, label, plot_colormap=True):
    ...
    print(
        "Difference in mean x={:.3f}, y={:.3f}".format(
...
if __name__ == "__main__":
    ...
    Ps = np.array(
        [
            [[2.85841814, 0.71772898], [0.71772898, 0.93786824]],
            [[3.28939458, 0.52634978], [0.52634978, 0.13435503]],
            [[2.40532661, 0.29692055], [0.29692055, 0.07671416]],
            [[2.23084082, 0.27823192], [0.27823192, 0.07488681]],
        ]
    )

    Ms = np.array(
        [
            [0.68040795, 0.17084572],
