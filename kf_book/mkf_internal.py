...
def plot_track_ellipses(N, zs, ps, cov, title):
    ...
    for i, p in enumerate(cov):
        plot_covariance_ellipse(
            (i + 1, ps[i]), cov=p, variance=4, axis_equal=False, ec="g", alpha=0.5
        )
...
def show_position_prediction_chart():
    ...
    ax.annotate(
        ...
        xy=(4, 4),
        xytext=(3, 3),
        arrowprops=dict(arrowstyle="->", ec="g", shrinkA=6, shrinkB=5, lw=3),
    )
    plt.show()
...
def show_x_error_chart(count):
    ...
    if count == 2:
        stats.plot_covariance_ellipse(
            (0, 0), ellipse=vel_ellipse, variance=sigma, edgecolor="r"
        )
    ...
    if count == 4:
        ...
        stats.plot_covariance_ellipse(
            (0, 0), ellipse=vel_ellipse, variance=sigma, edgecolor="r", alpha=0.25
        )
...
def show_x_with_unobserved():
    ...
    isct = Ellipse(xy=(2, 2), width=0.2, height=1.2, edgecolor="r", fc="None", lw=4)
...
def plot_correlation_covariance():
    P = [[4, 3.9], [3.9, 4]]
    plot_covariance_ellipse((5, 10), P, edgecolor="k", variance=[1, 2**2, 3**2])
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.gca().autoscale(tight=True)
...
if __name__ == "__main__":
    # show_position_chart()
    plot_3d_covariance((2, 7), np.array([[8.0, 0], [0, 1.0]]))
